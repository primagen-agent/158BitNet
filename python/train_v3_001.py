"""V3-001 local trainer: supervision-completion arm, 50 steps, cached C features.
...
REG = RESEARCH / 'experiments/V3-001-proposal.json'
...
def main():
    ...
    if not reg.get('user_approval', {}).get('approved'): raise ValueError('V3-001 not approved')
    steps = reg['budget']['steps']; ckpt_every = reg['budget']['steps_per_checkpoint']
    out = ROOT / 'build/neural-memory-v3-001'; out.mkdir(parents=True, exist_ok=True)
    fm = json.loads((ROOT / 'build/neural-memory-cg003-full-features/manifest.json').read_text())
    ...
    report = {'format': 'v3-001-train-report-v1', 'registration_sha256': sha_file(REG),
