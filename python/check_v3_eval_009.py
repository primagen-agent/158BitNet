...
def run(a):
    ...
    tr = json.loads((ROOT / 'build/neural-memory-v3-013/train-report.json').read_text())
    ...
    binding = ModelBinding(BACKBONE_SHA256, fm['tokenizer_sha256'], fm['encoder_identity']['encoder_id'],
                           sha_file(RESEARCH / 'experiments/EV-009.json'))
    ...
    result = {'format': 'ev009-candidate-eval-v1', 'registration_sha256': sha_file(RESEARCH / 'experiments/EV-001.json'),
