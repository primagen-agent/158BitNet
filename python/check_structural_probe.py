"""DG-029 balanced mode probe + structural-scalar route probe. Zero model updates."""
...
with torch.no_grad():
    for cid in ('JB-001','JB-002'):
        ...
        for meta in data['index'].values():
            ...
            structural=torch.tensor([float(len(x.source)>0),float(len(spans)),float(int(x.allowed.sum()))/32.0])
            route_z[-1]=torch.cat((route_z[-1],structural))
            if t.static_targets.route==1:
                for i,lab in enumerate(t.idle_targets):
                    if lab is None:continue
                    p=t.prefix(i,purpose='training_diagnostic')
                    mode_z.append(torch.cat((hidden[pindex[p]],q,evidence)));mode_y.append(int(lab))
codec.close()
Zall=torch.stack(route_z);Y=torch.tensor(route_y)
mu=Zall.mean(0,keepdim=True);sd=Zall.std(0,keepdim=True).clamp_min(1e-6)
Z=(Zall-mu)/sd
Mall=torch.stack(mode_z);My=torch.tensor(mode_y)
mmu=Mall.mean(0,keepdim=True);msd=Mall.std(0,keepdim=True).clamp_min(1e-6)
M=(Mall-mmu)/msd
...
mode_counts=Counter(int(v) for v in My.tolist())
out={'format':'dg029-balanced-structural-probe-v1','registration_sha256':sha_file(RESEARCH/'experiments/DG-029.json'),
 ...
 'mode_class_counts':dict(mode_counts),'mode_probe_cv_recall':mode_res,'mode_actual_note':'start never predicted: 0/268',
 'route_structural_augmented':True,'features_standardized':True,
 'frozen_reader_digest':reader_digest,'optimizer_steps':0}
p=RESEARCH/'checks/DG-029-probe.json'
