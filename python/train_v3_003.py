"""V3-003 local trainer (structural route input + detached mode): supervision-completion arm, 50 steps, cached C features.
...
REG = RESEARCH / 'experiments/V3-003-proposal.json'
...
def main():
    ...
    if not reg.get('user_approval', {}).get('approved'): raise ValueError('V3-003 not approved')
    steps = reg['training']['steps_requested']; ckpt_every = 10
    out = ROOT / 'build/neural-memory-v3-003'; out.mkdir(parents=True, exist_ok=True)
    fm = json.loads((ROOT / 'build/neural-memory-cg003-full-features/manifest.json').read_text())
    ...
    reader = make_struct_route_reader(digest(vars(binding)), width=64, seed=1018).eval()
    ...
    report = {'format': 'v3-003-train-report-v1','struct_mu':list(STRUCT_MU),'struct_sd':list(STRUCT_SD), 'registration_sha256': sha_file(REG),
