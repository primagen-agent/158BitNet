...
binding=ModelBinding(BACKBONE_SHA256,fm['tokenizer_sha256'],fm['encoder_identity']['encoder_id'],sha_file(RESEARCH/'experiments/V3-018-proposal.json'))
...
outdir=ROOT/'build/neural-memory-v3-018';outdir.mkdir(parents=True,exist_ok=True)
canon_h={}
...
for step in range(1,101):
    v=opt.step(closure)
    if step%20==0: print(json.dumps({'step':step,'loss':round(float(v),3)}),flush=True)
torch.save({'step':100,'reader':reader.state_dict(),'branch':branch.state_dict()},outdir/'ckpt-100.pt')
