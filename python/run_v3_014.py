...
binding=ModelBinding(BACKBONE_SHA256,fm['tokenizer_sha256'],fm['encoder_identity']['encoder_id'],sha_file(RESEARCH/'experiments/V3-014-proposal.json'))
...
outdir=ROOT/'build/neural-memory-v3-014';outdir.mkdir(parents=True,exist_ok=True)
canon_h={}
    ...
    for it in items:
        if it['route']==3:
            ...
            if it.get('canon'):
                ...
                loss=fl['route']+8.0*(fl['fact']+fl['value']+fl['boundary'])
                # contrastive: push down the distractor span (other city in world_cities)
...
torch.save({'step':450,'reader':reader.state_dict(),'branch':branch.state_dict()},outdir/'ckpt-450.pt')
