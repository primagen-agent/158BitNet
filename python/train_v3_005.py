"""V3-005 local trainer (supervision-defect fix): supervision-completion arm, 50 steps, cached C features.
...
REG = RESEARCH / 'experiments/V3-005-proposal.json'
...
def main():
    ...
    if not reg.get('user_approval', {}).get('approved'): raise ValueError('V3-005 not approved')
    steps = reg['training']['steps_requested']; ckpt_every = 10
    out = ROOT / 'build/neural-memory-v3-005'; out.mkdir(parents=True, exist_ok=True)
    fm = json.loads((ROOT / 'build/neural-memory-cg003-full-features/manifest.json').read_text())
    ...
    for step in range(1, steps + 1):
        ...
        for it in batch:
            if it['route'] == 2:
                ...
                # V3-005 fix: route-2 items ALSO receive the reader static supervision
    ...
    report = {'format': 'v3-005-train-report-v1', 'registration_sha256': sha_file(REG),
