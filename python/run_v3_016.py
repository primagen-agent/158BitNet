...
binding=ModelBinding(BACKBONE_SHA256,fm['tokenizer_sha256'],fm['encoder_identity']['encoder_id'],sha_file(RESEARCH/'experiments/V3-016-proposal.json'))
...
outdir=ROOT/'build/neural-memory-v3-016';outdir.mkdir(parents=True,exist_ok=True)
canon_h={}
for cid in ('JB-001','JB-002'):
    ...
    for meta in data['index'].values():
        ...
        if t.static_targets.route==1:
            for si2,l2 in enumerate(t.idle_targets):
                if l2 is None: continue
                pre_toks=t.prompt_ids+t.completion_ids[:si2]
                text=b''.join(codec.decode_pieces(list(pre_toks))).decode('utf-8',errors='ignore')
                cp=tuple(codec.tokenizer.encode(text.rstrip(),True))
                if len(cp)<=len(t.prompt_ids): continue
                if cp not in canon_h:
                    try:
                        ...
                        canon_h[cp]=torch.from_numpy(ref['hidden'].copy());canon+=1
                    except Exception: continue
                ...
                ent2={'x':x,'layout':layout,'traj':t,'route':1,'key':key,'rid':rid+'-canon','static_prefix':cp,'static_hidden':canon_h[cp],
                      'idle':[(si2,l2,cp,canon_h[cp])],'canon':True,'canon_pos':si2}
                items.append(ent2)
    ...
    for it in items:
        if it['route']==3:
            ...
            for i,lab,p,h in it['idle']:
                if it.get('canon') and lab!=1: continue  # mode CE only at START canonical positions
...
torch.save({'step':470,'reader':reader.state_dict(),'branch':branch.state_dict()},outdir/'ckpt-470.pt')
