"""V3-002 local trainer (loss-composition fix): supervision-completion arm, 50 steps, cached C features.
...
REG = RESEARCH / 'experiments/V3-002-proposal.json'
...
def main():
    ...
    if not reg.get('user_approval', {}).get('approved'): raise ValueError('V3-002 not approved')
    steps = reg['training']['steps_requested']; ckpt_every = 10
    out = ROOT / 'build/neural-memory-v3-002'; out.mkdir(parents=True, exist_ok=True)
    fm = json.loads((ROOT / 'build/neural-memory-cg003-full-features/manifest.json').read_text())
    ...
    for step in range(1, steps + 1):
        ...
        for it in batch:
            if it['route'] == 2:
                ...
                loss = torch.stack(per).sum() / len(per)
            else:
                ...
                total = 4.0 * static['route'] + (static['total'] - static['route'])
                if it['idle']:
                    idle_n = len(it['idle'])
                    for i, lab2, p, h in it['idle']:
                        out2 = reader(it['x'], it['layout'], PrefixFeature(p, h, it['key']), p)
                        w = 4.0 if lab2 == 1 else 1.0
                        total = total + w * F.cross_entropy(out2.mode_logits[None], torch.tensor([lab2])) / idle_n
                loss = total
    ...
    report = {'format': 'v3-002-train-report-v1', 'registration_sha256': sha_file(REG),
