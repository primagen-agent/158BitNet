...
def load_features(cache_path, raw_path, writer):
    ...
    return torch.stack(x), torch.tensor(y), set(worlds)


@torch.inference_mode()
def evaluate(head, x, y):
    prediction = head(x).argmax(dim=-1)
    return {name: (prediction[y == value] == value).float().mean().item()
...
def main():
    ...
    tx, ty, train_ids = load_features(args.train_features, args.train_jsonl, writer)
    vx, vy, valid_ids = load_features(args.valid_features, args.valid_jsonl, writer)
    ...
    pools = [(ty == value).nonzero().flatten() for value in (0, 1)]
    best, selected, state = None, 0, None
    for step in range(1, args.steps + 1):
        indices = torch.cat([pool[torch.randint(len(pool), (64,), device=args.device)] for pool in pools])
        loss = F.cross_entropy(head(tx[indices]), ty[indices])
        optimizer.zero_grad(); loss.backward(); optimizer.step()
        if step % 50 == 0:
            metrics = evaluate(head, vx, vy)
            key = (min(metrics.values()), sum(metrics.values()))
            if best is None or key > (min(best.values()), sum(best.values())):
