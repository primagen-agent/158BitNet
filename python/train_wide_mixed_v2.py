...
def main():
    ...
    fit_decl = [m for m in fit if not m['dialogue']]
    fit_dial = [m for m in fit if m['dialogue']]
    ...
    for step in range(1, 3001):
        opt.zero_grad()
        half = min(8, len(fit_decl), len(fit_dial))
        batch = rng.sample(fit_decl, half) + rng.sample(fit_dial, half)
        total, n = 0., 0
        for m in batch:
    ...
    out_dir = ROOT / 'build/neural-memory-ec006-wide-v2'
