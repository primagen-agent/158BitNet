...
binding=ModelBinding(BACKBONE_SHA256,fm['tokenizer_sha256'],fm['encoder_identity']['encoder_id'],sha_file(RESEARCH/'experiments/V3-019-proposal.json'))
...
outdir=ROOT/'build/neural-memory-v3-019';outdir.mkdir(parents=True,exist_ok=True)
canon_h={}
...
for step in range(1,1001):
    batch=[]
    while len(batch)<32: batch.append(items[order[cursor%len(order)]]);cursor+=1
    opt.zero_grad();tot=0.
    for it in batch:
        if it['route']==3:
    ...
    if step%100==0:
        ...
        torch.save({'step':step,'reader':reader.state_dict(),'branch':branch.state_dict()},outdir/f'ckpt-{step:04d}.pt')
