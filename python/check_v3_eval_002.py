"""EV-002: V3-002 step-50 candidate free-reply evaluation (DG-023 protocol)."""
...
def run(a):
    reg = json.loads((RESEARCH / 'experiments/EV-002.json').read_text()) if (RESEARCH / 'experiments/EV-002.json').exists() else json.loads((RESEARCH / 'experiments/EV-001.json').read_text())
    ck = torch.load(ROOT / 'build/neural-memory-v3-002/ckpt-050.pt', weights_only=False)
    tr = json.loads((ROOT / 'build/neural-memory-v3-002/train-report.json').read_text())
    ...
    binding = ModelBinding(BACKBONE_SHA256, fm['tokenizer_sha256'], fm['encoder_identity']['encoder_id'],
                           sha_file(RESEARCH / 'experiments/V3-002-proposal.json'))
            ...
            for scenario in scenarios:
                for language in ('en', 'zh'):
                    ...
                    core = AutonomousValueController(reader, x, layout, snapshot,
                                                     ...
                                                     codec, prompt, max_new_tokens=BUDGET)
                    ...
                    rows.append({'id': rid, 'corpus': corpus, 'scenario': f'{scenario}/{language}',
                                 ...
                                 'parity_ok': parity_ok, 'utf8_tail_dropped': tail, 'reply': text})
    ...
    result = {'format': 'ev002-candidate-eval-v1', 'registration_sha256': sha_file(RESEARCH / 'experiments/EV-001.json'),
