"""CG-003 native adapter for policy-bound JointState; DG-012 stays unchanged."""
import json
...
from joint_memory_model import JointTokenMemory
...
class NativeJointBackend:
    def __init__(self, *, runtime, tokenizer, tok_probe, gguf, probe, reference_probe,
        ...
        if type(model) is not JointTokenMemory or type(enabled) is not bool:
    ...
    def __call__(self, prefix, sources):
        ...
        with torch.no_grad():
            ...
            route = int(self.state.core.state_logits.argmax()); qualification = None; mass = 0.
            ...
            if not self.steps and self.enabled:
                ...
                content, uncertainty = self.model.roles.branches(h, self.state.core.prepared)
                ...
                cm = mix_content_copy(cc, branches.positions, branches.copy_mass, self.state.core.pointer.inputs.token_ids)
            ...
            if not self.enabled or route == 0:
                ...
                delta = self.model.roles.content.residual(self.model.roles.layers - 1, h, self.state.core.prepared)
                ...
                output = mix_content_copy(content, pointer.position_probabilities, pointer.copy_mass, self.state.core.pointer.inputs.token_ids)
    ...
    def finish(self):
        ...
        for name in ('probe', 'reference_probe'):
            if sha_file(getattr(self, name)) != self.identity[name + '_sha256']: raise ValueError('native binary changed')
        report = {'identity': self.identity, 'enabled': self.enabled, 'route_policy': self.model.route_policy, 'steps': self.steps,
