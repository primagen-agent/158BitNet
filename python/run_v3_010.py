...
binding=ModelBinding(BACKBONE_SHA256,fm['tokenizer_sha256'],fm['encoder_identity']['encoder_id'],sha_file(RESEARCH/'experiments/V3-010-proposal.json'))
key=digest(vars(binding))
reader=make_struct_route_reader(key,width=64,seed=1018)
ck=torch.load(ROOT/'build/neural-memory-v3-009/ckpt-400.pt',weights_only=False)
...
ckb=torch.load(ROOT/'build/neural-memory-v3-009/ckpt-400.pt',weights_only=False)
branch.load_state_dict(ckb['branch'])
reader_digest=tensor_digest(reader.state_dict())
for n,p in reader.named_parameters():
    if not (n.startswith('route') or n.startswith('mode')): p.requires_grad_(False)
...
outdir=ROOT/'build/neural-memory-v3-010';outdir.mkdir(parents=True,exist_ok=True)
canon_h={}
        ...
        if t.static_targets.route==1:
            ...
            if si:
                ...
                if len(cp)>len(t.prompt_ids):
                    ...
                    ent2={'x':x,'layout':layout,'traj':t,'route':1,'key':key,'rid':rid+'-canon','static_prefix':p0,'static_hidden':hidden[pindex[p0]].clone(),
                          'idle':[(si,1,cp,canon_h[cp])]+[(i,0,t.prefix(i,purpose='training_diagnostic'),hidden[pindex[t.prefix(i,purpose='training_diagnostic')]].clone()) for i,l in enumerate(t.idle_targets) if l==0][:2]}
...
torch.save({'step':410,'reader':reader.state_dict(),'branch':branch.state_dict()},outdir/'ckpt-410.pt')
