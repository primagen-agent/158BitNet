...
binding=ModelBinding(BACKBONE_SHA256,fm['tokenizer_sha256'],fm['encoder_identity']['encoder_id'],sha_file(RESEARCH/'experiments/V3-011-proposal.json'))
...
ckb=torch.load(ROOT/'build/neural-memory-v3-010/ckpt-410.pt',weights_only=False)
...
outdir=ROOT/'build/neural-memory-v3-011';outdir.mkdir(parents=True,exist_ok=True)
canon_h={}
...
torch.save({'step':420,'reader':reader.state_dict(),'branch':branch.state_dict()},outdir/'ckpt-420.pt')
