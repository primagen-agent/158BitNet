...
def run(a):
    ...
    ck = torch.load(ROOT / 'build/neural-memory-v3-012/ckpt-430.pt', weights_only=False)
    tr = json.loads((ROOT / 'build/neural-memory-v3-012/train-report.json').read_text())
    ...
    binding = ModelBinding(BACKBONE_SHA256, fm['tokenizer_sha256'], fm['encoder_identity']['encoder_id'],
                           sha_file(RESEARCH / 'experiments/EV-008.json'))
    ...
    result = {'format': 'ev008-candidate-eval-v1', 'registration_sha256': sha_file(RESEARCH / 'experiments/EV-001.json'),
