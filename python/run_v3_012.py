...
binding=ModelBinding(BACKBONE_SHA256,fm['tokenizer_sha256'],fm['encoder_identity']['encoder_id'],sha_file(RESEARCH/'experiments/V3-012-proposal.json'))
key=digest(vars(binding))
reader=make_struct_route_reader(key,width=64,seed=1018)
ck=torch.load(ROOT/'build/neural-memory-v3-011/ckpt-420.pt',weights_only=False)
...
ckb=torch.load(ROOT/'build/neural-memory-v3-011/ckpt-420.pt',weights_only=False)
...
outdir=ROOT/'build/neural-memory-v3-012';outdir.mkdir(parents=True,exist_ok=True)
canon_h={}
for cid in ('JB-001','JB-002'):
    ...
    for meta in data['index'].values():
        ...
        if t.static_targets.route==2 and len([e for e in items if e.get('canon2')])<100:
            import numpy as _np
            for pi in range(min(3,len(t.completion_ids))):
    ...
    for it in items:
        if it['route']==3:
            ...
            if it.get('canon'):
                ...
                loss=fl['route']+4.0*(fl['fact']+fl['value']+fl['boundary'])
            else:
                loss=fine_loss(o,it['traj'].static_targets)['route']
...
torch.save({'step':430,'reader':reader.state_dict(),'branch':branch.state_dict()},outdir/'ckpt-430.pt')
