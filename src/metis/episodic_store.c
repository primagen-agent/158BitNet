...
int metis_episodic_add_with_key(
    metis_episodic_store_t *store, const char *text, const float *key) {
    return metis_episodic_add_with_priority(store, text, key, 0.0f);
}
...
int metis_episodic_set_priority(
    ...
    if (store == NULL || index >= store->count ||
        !isfinite(priority) || priority < 0.0f || priority > 1.0f)
        return -1;
    store->priorities[index] = priority;
    return 0;
}
...
int metis_episodic_delete_with_key(
    ...
    for (size_t next = index + 1; next < store->count; ++next) {
        ...
        store->priorities[next - 1] = store->priorities[next];
    }
    --store->count;
    store->records[store->count] = NULL;
    store->keys[store->count] = NULL;
    store->priorities[store->count] = 0.0f;
